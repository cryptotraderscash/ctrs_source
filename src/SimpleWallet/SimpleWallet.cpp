...
std::shared_ptr<WalletInfo> createViewWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    while (true)
    {
        ...
        else if (address.substr(0, 4) != "CTRS")
        {
            std::cout << WarningMsg("Invalid address! It should start with "
                                    "\"ctrs\"!") << std::endl;
                                                        ...
                                                        address))
        {
            std::cout << WarningMsg("Failed to parse CTRS address! Ensure you "
...
void help(bool viewWallet)
{
    std::cout << "Available commands:" << std::endl
              ...
              << "Display how much CTRS you have" << std::endl
    ...
    if (viewWallet)
    {
        std::cout << InformationMsg("Please note you are using a view only "
                                    "wallet, and so cannot transfer CTRS.")
                  << std::endl;
    }
    else
    {
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[CTRS " + shortName + "]: ");
}
...
void viewWalletMsg()
{
    std::cout << InformationMsg("Please remember that when using a view wallet "
              ...
              << std::endl << "This means if you received 100 CTRS and then "
              << "sent 50 CTRS, your balance would appear to still be 100 "
              << "CTRS." << std::endl
